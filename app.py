...
## Task Statistics
- **Total Tasks**: {task_stats['total_tasks']}
- **Completed Tasks**: {task_stats['completed_tasks']} ({completion_percentage:.1f}%)
- **Open Tasks**: {task_stats['open_tasks']}
- **Number of Lists**: {task_stats['lists_count']}
- **Number of Folders**: {task_stats['folders_count']}
...
@app.route('/generate_report_clickup/<space_id>', methods=['POST'])
def generate_report_clickup(space_id):
    api_token = session.get('api_token')
    if not api_token:
        flash('Please log in first', 'warning')
        return redirect(url_for('index_clickup'))
